...
表格按"入口触发者 → Prompt 装配 → History → Memory → Scope → Surface"顺序读。

| Path | 触发者 | 入口实现 | Prompt Profile | History Owner | Memory Injection Point | Scope Key | Streaming | Surface | Interruptable |
|---|---|---|---|---|---|---|---|---|---|
| **http-run** | HTTP `POST /runs` | `http.ts::/runs` → `service.ts::AgentCoreServiceHarness.run` | 按 model 规则 | checkpointer (`threadId`) | user-message | `threadId` | no | chat | no (non-streaming) |
| **http-run-stream** | HTTP `POST /runs/stream` | `http.ts::/runs/stream` → `service.ts::streamRun` | 按 model 规则 | checkpointer (`threadId`) | user-message | `chat:{threadId}` | SSE | chat | **yes** |
| **http-resume** | HTTP `POST /resume` + `/resume/stream` | `http.ts::/resume*` → `service.ts::resume` | 同原 run | checkpointer (`threadId`) | **none** | `chat:{threadId}` | optional SSE | chat | **yes** (streaming) |
| **cli-repl** | `npm run dev` (`cli.ts`) | `cli.ts::AgentCoreCli.start` | 按 model 规则 | checkpointer (`threadId` via `resolveInitialThreadId`) | user-message | `cli:{threadId}` | inline print | cli | no |
| **twitter-digest** | HTTP `POST /twitter/digest` | `twitter-digest.ts::generateDigest` (bypass) | **独立 system prompt** | none (single-turn) | **none** | request-scoped | no | digest | no |
| **heartbeat** | HeartbeatEngine tick / run-now | `engine.ts` → `service.runOnce` | 按 model 规则 | `heartbeat:{taskId}:{ts}` (single-turn) | user-message | `heartbeat:{taskId}` | no | heartbeat | no |
