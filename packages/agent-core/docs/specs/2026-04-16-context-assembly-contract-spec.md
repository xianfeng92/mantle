...
| **twitter-digest** | HTTP `POST /twitter/digest` | `twitter-digest.ts::generateDigest` (bypass agent loop) | **独立 system prompt**（`BASE_SYSTEM` + mode） | none (single-turn) | **none** | request-scoped (no reuse) | no |
